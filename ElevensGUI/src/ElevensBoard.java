...
public class ElevensBoard extends Board {
	...
	public boolean anotherPlayIsPossible() {
		...
		for (int i = 0; i < BOARD_SIZE - 1; i++) {
			for (int j = i+1; j < BOARD_SIZE; j++) {
				...
				list.add(i);
				list.add(j);
				if (containsPairSum11(list)) return true;
			}
		}
		for (int i = 0; i < BOARD_SIZE - 2; i++) {
			for (int j = i+1; j < BOARD_SIZE - 1; j++) {
				for (int k = j+1; k < BOARD_SIZE; k++) {
					...
					list.add(j);
					list.add(k);
					if (containsJQK(list)) return true;
				}
			}
		}
		return false;
	...
	private boolean containsJQK(List<Integer> selectedCards) {
		...
		for (int i : selectedCards) {
			if (cardAt(i).rank().equals("jack")) hasJ = true;
			if (cardAt(i).rank().equals("queen")) hasQ = true;
			if (cardAt(i).rank().equals("king")) hasK = true;
		}
